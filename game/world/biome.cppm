...
export namespace craftbuild {
	...
	struct BiomeRegistry {
		...
		inline static std::unordered_map<std::string, size> name2id;

		static none register_biome(const std::string& name, const Biome& biome) {
		...
		static std::string get_name(size biome_id) {
			if (registry.size() <= biome_id) return "";
			return registry[biome_id].name;
		}

		static size get_id(const std::string& biome_name) {
